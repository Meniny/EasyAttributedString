...
public protocol EAFontInfoAttribute {
	func featureSettings() -> [(type: Int, selector: Int)]
}

public extension EAFontInfoAttribute {
	
	func attributes() -> [[EAFontDescriptor.FeatureKey: Any]] {
		let featureSettings = self.featureSettings()
		return featureSettings.map {
			return [
				EAFontFeatureTypeIdentifierKey: $0.type,
				EAFontFeatureSelectorIdentifierKey: $0.selector,
			]
		}
	}
...
// MARK: - UIFont/NSFont with EAFontInfoAttribute

internal extension EAFont {
	...
	func withAttributes(_ attributes: [EAFontInfoAttribute]) -> EAFont {
		...
		var features = fontAttributes[EAFontDescriptorFeatureSettingsAttribute] as? [[EAFontDescriptor.FeatureKey: Any]] ?? []
		
		features.append(contentsOf: newFeatures)
		fontAttributes[EAFontDescriptorFeatureSettingsAttribute] = features
		
		let descriptor = EAFontDescriptor(fontAttributes: fontAttributes)
		#if os(OSX)
			return EAFont(descriptor: descriptor, size: pointSize)!
		#else
			return EAFont(descriptor: descriptor, size: pointSize)
		#endif
	}
...
public enum EANumberCase: EAFontInfoAttribute {
	case upper
	case lower
...
public enum EANumberSpacing: EAFontInfoAttribute {
	case monospaced
	case proportional
...
internal enum EAVerticalPosition: EAFontInfoAttribute {
	case normal
	case superscript
	case `subscript`
	case ordinals
...
/// - diagonal: Diagonal EAFractions, when written on paper, are written on one line
///				with the numerator diagonally above and to the left of the
///				demoninator, with the slash ("/") between them.
/// - vertical:	Vertical EAFractions, when written on paper, are written on one line
///				with the numerator directly above the demoninator, with a line lying horizontally between them.
public enum EAFractions: EAFontInfoAttribute {
	case disabled
	case diagonal
	case vertical
...
public enum EASmallCaps: EAFontInfoAttribute {
	case disabled
	case fromUppercase
	case fromLowercase
...
public struct EAStylisticAlternates {
	var one: Bool?
	var two: Bool?
	var three: Bool?
	var four: Bool?
...
extension EAStylisticAlternates {
	
	public static func one(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.one = isOn
		return alts
	}
	
	public static func two(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.two = isOn
		return alts
	}
	
	public static func three(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.three = isOn
		return alts
	}
	
	public static func four(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.four = isOn
		return alts
	}
	
	public static func five(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.five = isOn
		return alts
	}
	
	public static func six(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.six = isOn
		return alts
	}
	
	public static func seven(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.seven = isOn
		return alts
	}
	
	public static func eight(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.eight = isOn
		return alts
	}
	
	public static func nine(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.nine = isOn
		return alts
	}
	
	public static func ten(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.ten = isOn
		return alts
	}
	
	public static func eleven(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.eleven = isOn
		return alts
	}
	
	public static func twelve(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.twelve = isOn
		return alts
	}
	
	public static func thirteen(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.thirteen = isOn
		return alts
	}
	
	public static func fourteen(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.fourteen = isOn
		return alts
	}
	
	public static func fifteen(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.fifteen = isOn
		return alts
	}
	
	public static func sixteen(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.sixteen = isOn
		return alts
	}
	
	public static func seventeen(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.seventeen = isOn
		return alts
	}
	
	public static func eighteen(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.eighteen = isOn
		return alts
	}
	
	public static func nineteen(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.nineteen = isOn
		return alts
	}
	
	public static func twenty(on isOn: Bool) -> EAStylisticAlternates {
		var alts = EAStylisticAlternates()
		alts.twenty = isOn
		return alts
	}
...
extension EAStylisticAlternates {
	...
	mutating public func add(other theOther: EAStylisticAlternates) {
	...
	public func byAdding(other theOther: EAStylisticAlternates) -> EAStylisticAlternates {
		var varSelf = self
...
extension EAStylisticAlternates: EAFontInfoAttribute {
...
public func + (lhs: EAStylisticAlternates, rhs: EAStylisticAlternates) -> EAStylisticAlternates {
	return lhs.byAdding(other: rhs)
}

//MARK: - EAContextualAlternates

/// Different contextual alternates available for customizing a font. Not
/// all fonts support all (or any) of these options.
public struct EAContextualAlternates {
...
// Convenience constructors
extension EAContextualAlternates {
	
	public static func contextualAlternates(on isOn: Bool) -> EAContextualAlternates {
		var alts = EAContextualAlternates()
		alts.contextualAlternates = isOn
		return alts
	}
	
	public static func swashAlternates(on isOn: Bool) -> EAContextualAlternates {
		var alts = EAContextualAlternates()
		alts.swashAlternates = isOn
		return alts
	}
	
	public static func contextualSwashAlternates(on isOn: Bool) -> EAContextualAlternates {
		var alts = EAContextualAlternates()
...
extension EAContextualAlternates {
	...
	mutating public func add(other theOther: EAContextualAlternates) {
	...
	public func byAdding(other theOther: EAContextualAlternates) -> EAContextualAlternates {
		var varSelf = self
		varSelf.add(other: theOther)
		return varSelf
	}
...
extension EAContextualAlternates: EAFontInfoAttribute {
...
public func + (lhs: EAContextualAlternates, rhs: EAContextualAlternates) -> EAContextualAlternates {
	return lhs.byAdding(other: rhs)
}

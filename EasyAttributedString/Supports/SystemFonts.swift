...
public enum EASystemFonts: String, EAFontConvertible {
	...
	public func font(size: CGFloat?) -> EAFont {
		#if os(tvOS)
		return EAFont(name: self.rawValue, size: (size ?? TVOS_SYSTEMFONT_SIZE))!
		#elseif os(watchOS)
		return EAFont(name: self.rawValue, size: (size ?? WATCHOS_SYSTEMFONT_SIZE))!
		#else
		return EAFont(name: self.rawValue, size: (size ?? EAFont.systemFontSize))!
		#endif
	}

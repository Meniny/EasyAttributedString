...
/// EAStyle class encapsulate all the information about the attributes you can apply to a text.
public class EAStyle: EAStyleProtocol {
	
	//MARK: - INTERNAL PROPERTIES

	/// Handler to initialize a new style.
	public typealias StyleInitHandler = ((EAStyle) -> (Void))
	...
	/// configurable attributes are exposed at `EAStyle` level.
	private var fontInfo: EAFontInfo = EAFontInfo()
	...
	/// You can pass any `EAFontConvertible` conform object, it will be transformed to a valid `UIFont`/`NSFont``
	/// and used by the style itself. Both `String`, `EASystemFonts` and `UIFont`/`NSFont` are conform to this protocol yet
	/// so you are able to pass a valid font as a string, from predefined list or directly as an instance.
	public var font: EAFontConvertible {
		set {
			self.fontInfo.font = newValue
			if let f = newValue as? EAFont {
	...
	/// You can pass any `EAColorConvertible` conform object, it will be transformed to a valid `UIColor`/`NSColor`
	/// automatically. Both `UIColor`/`NSColor` and `String` are conform to this protocol.
	public var color: EAColorConvertible? {
	...
	/// You can pass any `EAColorConvertible` conform object, it will be transformed to a valid `UIColor`/`NSColor`
	/// automatically. Both `UIColor`/`NSColor` and `String` are conform to this protocol.
	public var backColor: EAColorConvertible? {
	...
	public var underline: (style: NSUnderlineStyle?, color: EAColorConvertible?)? {
		set {
		...
		get {
			...
			let color: EAColor? = self.get(attributeForKey: .underlineColor)
	...
	/// Value must be a tuple which define the style of the line (as `NSUnderlineStyle`)
	/// and the optional color of the line (if `nil`, foreground color is used instead).
	public var strikethrough: (style: NSUnderlineStyle?, color: EAColorConvertible?)? {
		set {
		...
		get {
			...
			let color: EAColor? = self.get(attributeForKey: .strikethroughColor)
			return (style?.toUnderlineStyle(),color)
		}
	}
	...
	public var lineBreakMode: EALineBreak {
	...
	public var ligatures: EAStringLigatures? {
		set {
		...
		get {
			...
			return EAStringLigatures(rawValue: value.intValue)
		}
	}
	...
	public var numberCase: EANumberCase? {
	...
	public var smallCaps: Set<EASmallCaps> {
	...
	public var stylisticAlternates: EAStylisticAlternates {
	...
	public init(_ handler: StyleInitHandler? = nil) {
		...
		#if os(tvOS)
		self.set(attribute: EAFont.systemFont(ofSize: TVOS_SYSTEMFONT_SIZE), forKey: .font)
		#elseif os(watchOS)
		self.set(attribute: EAFont.systemFont(ofSize: WATCHOS_SYSTEMFONT_SIZE), forKey: .font)
		#else
		self.set(attribute: EAFont.systemFont(ofSize: EAFont.systemFontSize), forKey: .font)
		#endif
		handler?(self)
	}
	...
	/// Initialize a new EAStyle by cloning an existing style.
	///
	/// - Parameter style: style to clone
	public init(style: EAStyle) {
		self.fontInfo.style = self
	...
	public func byAdding(_ handler: StyleInitHandler) -> EAStyle {
		let styleCopy = EAStyle(style: self)
		handler(styleCopy)
		return styleCopy
	}
}
